...
export class LineService {
  ...
  async sendAsStaff(user: RequestUser, residentId: string, template: string, payload: Record<string, unknown>) { const resident = await this.prisma.resident.findFirst({ where: { id: residentId, storeId: user.storeId, deletedAt: null } }); if (!resident) throw new NotFoundException('Resident not found'); assertBranchAccess(user, resident.branchId); const result = await this.sendToResident(user.storeId, resident.branchId, residentId, template, payload); if (template === 'staff-chat' && result.status === 'SENT') { const identity = await this.prisma.lineIdentity.findFirst({ where: { residentId } }); if (identity && typeof payload.message === 'string') await this.prisma.chatMessage.create({ data: { lineIdentityId: identity.id, direction: 'OUTBOUND', text: payload.message, lineMessageId: result.externalId ?? undefined } }); } return result; }
  async conversations(user: RequestUser, residentId: string) { const resident = await this.prisma.resident.findFirst({ where: { id: residentId, storeId: user.storeId, deletedAt: null }, include: { lineIdentity: true } }); if (!resident) throw new NotFoundException('Resident not found'); assertBranchAccess(user, resident.branchId); if (!resident.lineIdentity) return []; return this.prisma.chatMessage.findMany({ where: { lineIdentityId: resident.lineIdentity.id }, orderBy: { createdAt: 'asc' } }); }
  async handleWebhook(integrationId: string, body: { events?: Array<{ type?: string; source?: { userId?: string }; message?: { id?: string; type?: string; text?: string }}> }) { for (const event of body.events ?? []) { if (event.type !== 'message' || event.message?.type !== 'text' || !event.message.text || !event.source?.userId) continue; const identity = await this.prisma.lineIdentity.findFirst({ where: { lineUserId: event.source.userId, lineIntegrationId: integrationId } }); if (identity) await this.prisma.chatMessage.create({ data: { lineIdentityId: identity.id, direction: 'INBOUND', text: event.message.text, lineMessageId: event.message.id } }); } }
  async quota(user: RequestUser, branchId: string) { assertBranchAccess(user, branchId); const integration = await this.prisma.lineIntegration.findFirst({ where: { branchId, isActive: true } }); if (!integration) return { configured: false, usage: null, quota: null, remaining: null }; const token = this.credentials.decrypt(integration.channelAccessTokenEncrypted); const headers = { Authorization: `Bearer ${token}` }; const [quotaResponse, usageResponse] = await Promise.all([fetch('https://api.line.me/v2/bot/message/quota', { headers }), fetch('https://api.line.me/v2/bot/message/quota/consumption', { headers })]); if (!quotaResponse.ok || !usageResponse.ok) throw new NotFoundException('LINE quota unavailable'); const quota = await quotaResponse.json() as { value?: number }; const usage = await usageResponse.json() as { totalUsage?: number }; const limit = Number(quota.value ?? 0); const used = Number(usage.totalUsage ?? 0); return { configured: true, usage: used, quota: limit, remaining: Math.max(0, limit - used) }; }
