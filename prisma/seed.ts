...
async function main(): Promise<void> {
  ...
  await prisma.lineIntegration.upsert({ where: { branchId: branch.id }, create: { branchId: branch.id, displayName: 'Demo LINE OA', channelAccessTokenEncrypted: encryptSeedCredential(accessToken), miniAppChannelSecretEncrypted: encryptSeedCredential(process.env.SEED_LINE_MINIAPP_CHANNEL_SECRET ?? channelSecret), miniAppChannelId: process.env.SEED_LINE_MINIAPP_CHANNEL_ID ?? process.env.SEED_LINE_LOGIN_CHANNEL_ID ?? 'demo-miniapp-channel-id', messagingChannelSecretEncrypted: encryptSeedCredential(process.env.SEED_LINE_MESSAGING_CHANNEL_SECRET ?? channelSecret), messagingChannelId: process.env.SEED_LINE_MESSAGING_CHANNEL_ID ?? 'demo-messaging-channel-id', liffId: process.env.SEED_LINE_LIFF_ID ?? 'demo-liff-id' }, update: {} });
  ...
  await prisma.user.upsert({ where: { storeId_email: { storeId: store.id, email: 'platform@demo.local' } }, create: { storeId: store.id, roleId: platformRole.id, email: 'platform@demo.local', passwordHash: await hash('Platform123!', 12), displayName: 'Platform Super Admin', allBranches: true, isPlatformAdmin: true }, update: { roleId: platformRole.id, allBranches: true, isPlatformAdmin: true } });
